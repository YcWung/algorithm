...
template <typename Scalar, typename Index, int Dim> struct KdTreeNode
{
    struct BuildInfo
    {
        Scalar *pts;
...
struct KdTreeLeafNode : public KdTreeNode<Scalar, Index, Dim>
{
    virtual ~KdTreeLeafNode() = default;
    Index start, num;
};

template <typename ScalarT, int Dim, typename IndexT> class KdTree
{
public:
    using Node = KdTreeNode<ScalarT, Dim, IndexT>;

    KdTree(ScalarT *points_data, IndexT points_num, IndexT points_step)
        : data(points_data), num(points_num), step(points_step)
    {
        indices.resize(num);
    ...
    void knn(ScalarT *pt, IndexT k, std::vector<IndexT> &k_indices,
...
protected:
    ...
    void build(Node *node, int d, IndexT start, IndexT num)
    {
        ...
        ScalarT min = std::numeric_limits<ScalarT>::max(),
                max = std::numeric_limits<ScalarT>::min();
        for (IndexT i = 0; i < num; ++i)
        {
            ScalarT val = at(i, d);
            if (val < min)
                  ...
                  std::vector<ScalarT> &k_sqr_distances)
    {
        if (node->type == Node::Leaf)
        {
            ...
            std::sort(iv.begin(), iv.end(),
                      [&dv](IndexT x, IndexT y) { return dv[x] < dv[y]; });
            for (IndexT i = 0; i < num; ++i)
            {
                dv2[i] = dv[iv[i]];
                iv2[i] = indices[s + i];
        ...
        for (int i = 0; i < 2; ++i)
        {
            knn_impl(pt, k, node->contents.children[i], indices[i],
                     distances[i]);
        }
